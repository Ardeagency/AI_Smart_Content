 ...
 * {
 *   ok: true,
 *   stale: bool,          // si computed_at > 1h, el cliente debe lanzar sync
 *   brand: { nombre_marca, logo_url },
 *   meta_integration: { connected, account_name, picture, token_expires_at },
 *   dimensions: {
 *     A_activity:   { snapshot, posts_sample, heatmap },
 *     B_narrative:  { pillars_active, pillars_orphan, top_tones, fatigue_posts },
 *     C_retail:     { prices_count, alerts },              // desde retail_prices
 *     D_sentiment:  { emotion_distribution, top_posts },
 *     E_diagnostic: { vulnerabilities, coherence_avg, clarity_avg }
 *   }
 * }
 */
...
exports.handler = async (event) => {
  ...
  // Detectar si datos están desactualizados
  ...
  const topTones = Object.entries(toneCount)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 4)
    .map(([tone, count]) => ({ tone, count, pct: Math.round(count / analyses.length * 100) }));

  const B_narrative = {
    pillars_active:  pillars.filter(p => p.pillar_type === 'active'),
    pillars_orphan:  pillars.filter(p => p.pillar_type === 'orphan'),
    top_tones:       topTones,
    emotion_distribution: Object.entries(emotionCount)
      .sort((a, b) => b[1] - a[1])
  ...
  // ── Dimensión C: Retail (MAP Monitor) ────────────────────────────────────
  const pricesByRetailer = {};
  retail.forEach(r => {
    if (!pricesByRetailer[r.retailer]) pricesByRetailer[r.retailer] = [];
    pricesByRetailer[r.retailer].push(r);
  });
  ...
  const C_retail = {
    ...
    alerts:        retail.filter(r => r.stock_status === 'out_of_stock').map(r => ({
      retailer: r.retailer,
  ...
  // ── Dimensión D: Sentimiento ──────────────────────────────────────────────
  // Top posts por engagement, cruzados con análisis
  const analysisMap = {};
  ...
  const D_sentiment = {
    ...
    top_posts: topPosts,
    clarity_avg: analyses.length > 0
      ? Math.round(analyses.reduce((s, a) => s + (a.clarity_score || 0), 0) / analyses.length)
      : null
  };

  // ── Dimensión E: Diagnóstico (SWOT) ───────────────────────────────────────
  const E_diagnostic = {
    vulnerabilities:  vulns,
    coherence_avg:    B_narrative.coherence_avg,
    clarity_avg:      D_sentiment.clarity_avg,
  ...
  return {
    ...
    body: JSON.stringify({
      ...
      period,
      dimensions: { A_activity, B_narrative, C_retail, D_sentiment, E_diagnostic }
    })
  };
};
