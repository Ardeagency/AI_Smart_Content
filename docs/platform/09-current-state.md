...
🔴 **Roto / vacío:**
- ~~`body_missions` colgadas tipo `competitor_signal_analysis`~~ ✅ resuelto 2026-05-05 (BUG-001 opción A): flujo eliminado en `signal-webhook.controller.js` + `audience-alignment.service.js`, 46 missions cerradas + 3 pending_actions expiradas.
...
#### Bug 1 — Body missions colgadas ✅ RESUELTO 2026-05-05
**Causa raíz confirmada:** la migración Apify del 2026-04-28 ("droplegacy") removió los handlers de `competitor_signal_analysis` y `execute_update_persona` del `action-executor.service.js`, pero NO removió la creación río arriba en `signal-webhook.controller.js:124` ni las pending_actions tipo `update_persona` en `audience-alignment.service.js`. Las missions caían en `default → throw "unknown action_type"` y se quedaban en `pending` para siempre.

**Decisión de producto:** Opción A — eliminar el flujo. El nuevo modelo Apify ya enriquece signals vía `python-analyzer` (pysentimiento + KeyBERT) + `media_descriptions_cache` (Claude). El análisis per-post con Vera quedó deprecado; Vera ahora opera en BATCH desde Layer 4.

**Cambios aplicados:**
- `signal-webhook.controller.js`: la función `enqueueSignalAnalysis` ya no crea body_missions ni agent_queue_jobs/mission_runs. Solo conserva la creación de `brand_vulnerabilities` para signals HIGH/CRITICAL.
- `audience-alignment.service.js`: el bloque `if (score < 0.5) → INSERT vera_pending_actions{action_type:'update_persona'}` se eliminó. El score y el update del persona siguen ocurriendo.
- BD: 46 body_missions colgadas → `failed` (con `result_reference.error`); 3 vera_pending_actions tipo `update_persona` → `expired`.
- Servicio `ai-engine` reiniciado vía `systemctl restart ai-engine.service`.
...
### Alta prioridad
1. ~~Reparar competitor_signal_analysis dispatcher~~ ✅ resuelto 2026-05-05 (BUG-001 opción A).
