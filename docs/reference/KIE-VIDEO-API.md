...
- **input**: siempre un **objeto** (nunca string). Contiene `mode`, `sound`, `duration`, `aspect_ratio`; **`multi_shots`** (requerido, boolean): `false` = single-shot → se envía `prompt`; `true` = multi-shot → se envía `multi_prompt` (array de `{ prompt, duration }`, no vacío). `image_urls` solo si hay URLs; `kling_elements` solo si el prompt incluye `@element_name` y el elemento tiene ≥1 imagen (o 1 video).
...
- **Body enviado a KIE**: `input` siempre objeto. **multi_shots** es obligatorio (boolean): `false` → single-shot, se envía `prompt`; `true` → multi-shot, se envía `multi_prompt` (array no vacío de `{ prompt, duration }`). Nunca se deja `multi_shots` sin definir. `duration` 1–12; `aspect_ratio` 16:9|9:16|1:1. `kling_elements` solo referenciados con ≥1 imagen (o 1 video). Prompt truncado a 2500 caracteres.
