...
| [FEAT-025](./FEAT-025-mercadolibre-api-publica-fiche.md) | Ficha MercadoLibre via API publica: implementar `extractMercadoLibreId()` + `fetchFromMercadoLibreApi()` + branch en `api-products-generate-fiche` handlerImpl (hoy detecta la plataforma pero no la consume). |
