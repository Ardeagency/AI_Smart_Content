...
| [OPS-013](./OPS-013-triggers-atascados-sin-reagendar.md) | 🆕 🔴 **36 triggers `active` sin reagendar hace 7-8 dias** (de 9.557; el resto corre — 2.227 corrieron hoy). Tres casos: 33 cerraron `success` el 1-2/sep y nadie los volvio a agendar; `meta_ad_library_sync` **falla porque APIFY TOPO SU CUOTA** (50,36 USD sobre 50, plan STARTER, ciclo hasta el **2026-09-14**) — por eso `competitor_ads` se congelo el 21/08; y `strategic_review` (2) **NUNCA corrio**, 56 dias vencido. El aviso de Apify **SI disparo — 132 veces**, la ultima el mismo 10/09, y el throttle funciona: lo que falla es que **`developer_notifications` no tiene lectores**. Los otros 34 si estan mudos. |
| [OPS-014](./OPS-014-sensor-reporta-success-con-errores-dentro.md) | 🆕 🔴 **194 corridas en verde con errores dentro.** `mercadolibre_metrics` 108/108 y `shopify_metrics` 50/50 —el 100% de las suyas— cierran `success` con `stats.errors > 0` y las metricas en `null`; `google_ads_insights` 36 de 64. El estado miente: no se puede alertar sobre ellos ni confiar en su verde, y esconden el bloqueo real. |
...
| [OPS-006](./OPS-006-meta-ad-library-diagnostico.md) | ✅ **RESUELTA 2026-09-10 — se puede borrar.** La pregunta ya tiene respuesta: `competitor_ads` tiene **98** filas, todas `meta`, todas con creative, capturadas 03→21 de agosto. La via A/B/C se decidio en los hechos y **funciono**. Que se congelara el 21/08 NO es diagnostico de la Biblioteca: es que **Apify topo su cuota** → trasladado a OPS-013. |
...
| [DATA-001](./DATA-001-configure-competitor-entities.md) | ✅ **SIN ALCANCE PROPIO — cerrar.** De lo que denunciaba: `intelligence_entities` **37**, `competitor_ads` **98**, `visual_references` **2**. Solo `retail_prices` sigue en **0**, y no por falta de configurar competidores sino porque la API publica de MercadoLibre da **403** (verificado hoy). Lo que queda es **FEAT-025**; seguir el hilo alli. |
