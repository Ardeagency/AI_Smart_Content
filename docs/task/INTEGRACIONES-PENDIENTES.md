...
- 🔲 **Capa de PUBLICAR (`api-social-publish`) — hoy solo FB/IG real; YouTube/X/TikTok son STUB.** Gotchas reales detectados 2026-06-16 antes de construir: (1) **no testeable** hasta que una marca conecte la cuenta (ninguna red nueva conectada aún); (2) **TikTok `PULL_FROM_URL` exige dominio verificado** del video — el media sale de `supabase.co` (no verificado), no de `aismartcontent.io`; hay que servir el video por dominio verificado (proxy) **o** usar `FILE_UPLOAD` por chunks; (3) cada red trae su flujo de media propio (X media/upload, YouTube resumable). Meta funciona porque acepta la URL de supabase directa. **No construir a ciegas: requiere una cuenta conectada para validar.**
