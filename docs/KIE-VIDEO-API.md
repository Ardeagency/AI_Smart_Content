...
- **422 (Unprocessable Content)**: validación de parámetros fallida. El proxy envía `input`: `mode`, `prompt`, `sound`, `duration` (número 3–15), `aspect_ratio` (`16:9`|`9:16`|`1:1`), y si hay varios shots `multi_shots: true` y `multi_prompt: [{ prompt }]`. Revisar en consola del navegador el cuerpo de la respuesta (campo `kieBody`) para ver el mensaje de KIE. **Solución temporal:** en Netlify define la variable de entorno `KIE_VIDEO_MINIMAL_PAYLOAD=1`; entonces el proxy enviará solo `model` + `input: { mode, prompt }` (sin duration, aspect_ratio, sound, multi_*), lo que suele evitar 422 si el fallo viene de un parámetro opcional.
