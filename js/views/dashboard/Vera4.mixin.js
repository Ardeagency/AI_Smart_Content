/**
 * DashboardView — Vera4 mixin (las cards del cerebro de Vera, schema `cards.vera4`),
 * repartidas en las 4 pestañas del tablero. Portado de v1 (8520ff8a^) con su marcado, sus
 * clases y sus plantillas; lo que cambia es de dónde sale el dato y qué pasa cuando falta.
 *
 * DE DÓNDE SALEN: DashboardDatos.lectura(org, {scope, versiones:[4], periodo}) — la vista
 * marketing.lecturas_tablero (o, sin ella, el sobre de marketing.readings.evidence). Mi Marca pide
 * la del periodo activo del filtro; Competencia, Tendencias y Estrategia, la última.
 *
 * «TODAVÍA NO»: cada pestaña pinta TODAS las cards de su catálogo, en el orden del catálogo (el
 * mismo de v1). La que Vera escribió sale completa; la que no, sale en su sitio con su marco, su
 * rótulo y su ancho, y dice en palabras qué falta. Hoy solo hay lecturas vera4 de Mi Marca (nadie
 * escribe las de Competencia, Tendencias ni Estrategia desde el 07/08). Las de tab null
 * (recalibración, humildad, a2a) no tienen sitio y no se pintan. La Intuición de Competencia,
 * Tendencias y Estrategia es una banda aparte (`aparte`), con su propio «todavía no».
 *
 * REPARTO (VERA4[type].tab):
 *   Mi Marca    silencio · latencia · impacto_vs_ruido · emocion_objetivo · ritmo · autopsia ·
 *               victoria_explicada · causalidad · cobertura_momentos · rejilla_codigos ·
 *               deriva_codigos · construir_vs_cosechar · aplauso_vs_propagacion ·
 *               penetracion_vs_lealtad · biblioteca_patrones
 *   Competencia anomalia · error_ajeno · algoritmo_rival · territorio_tematico · registro_de_voz ·
 *               emocion_competencia · busqueda_vs_voz · supuesto_punto_ciego · proxima_movida
 *   Tendencias  pulso_nicho · senal_debil · triangulacion · tension · lo_que_falta ·
 *               crecimiento_categoria · tendencia_o_moda · tres_horizontes · derecho_a_jugar ·
 *               curva_adopcion (+ propuestas_fecha, aparte: solo si existe su hueco #tendPropuestas)
 *   Estrategia  decision_del_dia · autoridad_adn · puerta_aprobacion · produccion_viva ·
 *               pieza_asombro · formato · cadena_portafolio · verificacion · brief_humano ·
 *               bucle_outcome
 *
 * COLOR: las series categóricas salen del prisma de la plataforma (this._coloresPlataforma(),
 * orden fijo); el par divergente, de --color-error/--prisma-rojo ↔ --prisma-azul/--prisma-celeste.
 * Sin estilos en línea: anchos, alturas e intensidades viajan en data-* y los aplica _pintar.
 * SEGURIDAD: Vera lee posts, comentarios y web. TODO texto se escapa; un type desconocido se ignora.
 */
(function () {
  'use strict';
  ...
  /* Paletas: se resuelven de los tokens al empezar cada pintado (_v4Paleta). El orden de las
     categóricas es FIJO y nunca se cicla distinto: si un tono cambia de posición entre lecturas,
     el cliente tiene que reaprender el gráfico. */
  let SERIES = [];
  let DIV_NEG = [];   // rechazo · indiferencia
  let DIV_NEU = '';   // el centro tiene que leerse como "nada"
  let DIV_POS = [];   // interés · deseo · devoción
  let GRIS = '';      // las series que no se destacan

  /* Qué dice una card o una Intuición que Vera todavía no escribió, por pestaña. */
  const FALTA_TXT = {
    mi_marca:   () => __('Aparece cuando Vera vuelva a leer tu marca.'),
    monitoreo:  () => __('Aparece cuando Vera lea a tu competencia.'),
    tendencias: () => __('Aparece cuando Vera lea tu mercado.'),
    estrategia: () => __('Aparece cuando Vera proponga la jugada.'),
  };

  /* Ancho de cada plantilla: las de juicio largo y los instrumentos van a fila completa. */
  const ANCHAS = ['pulso', 'decision', 'ensayo', 'triang', 'cadena',
    'heatmap', 'apilada', 'divergente', 'indexadas',
    'enfasis', 'dispersion', 'lineas', 'balanza', 'patrones',
    'descomposicion', 'trendmoda', 'horizontes'];

  Object.assign(DashboardView.prototype, {

    /* Resuelve las paletas de los tokens (una vez por pintado: el tema de la marca puede cambiar). */
    _v4Paleta() {
      SERIES = this._coloresPlataforma();
      DIV_NEG = [this._tok('--color-error'), this._tok('--prisma-rojo')];
      DIV_NEU = this._tok('--white-20');
      const azul = this._tok('--prisma-azul');
      DIV_POS = [this._tok('--prisma-celeste'), this._rgba(azul, 0.75), azul];
      GRIS = this._tok('--white-20');
    },

    /* ── Carga ────────────────────────────────────────────────────────────
       Una consulta por pestaña (y por periodo en Mi Marca), cacheada por pintado: la piden dos
       pintores, la rejilla y la banda de Intuición. Nunca lanza: devuelve { datos, falta }. */
    async _loadVera4(scope) {
      this._vera4Cache = this._vera4Cache || {};
      const periodo = scope === 'mi_marca' && typeof this._veraPeriodoActivo === 'function' ? this._veraPeriodoActivo() : null;
      const clave = periodo ? `${scope}:${periodo}` : scope;
      if (Object.prototype.hasOwnProperty.call(this._vera4Cache, clave)) return this._vera4Cache[clave];
      const r = await this._loadVera4Fresh(scope, periodo);
      this._vera4Cache[clave] = r;
      return r;
    },

    async _loadVera4Fresh(scope, periodo) {
      if (!window.DashboardDatos || !this._orgId) return { datos: null, falta: 'error' };
      try {
        const r = await window.DashboardDatos.lectura(this._orgId, { scope, versiones: [4], periodo });
        const f = r.datos;
        const reading = f && f.reading;
        if (r.falta || !reading || reading.schema !== 'cards.vera4' || !Array.isArray(reading.cards)) return { datos: null, falta: r.falta };
        return { datos: { cards: reading.cards, createdAt: f.created_at }, falta: null };
      } catch (e) {
        console.warn('[Vera4] lectura no disponible:', e && e.message);
        return { datos: null, falta: 'error' };
      }
    },

    /* ── Pintado ──────────────────────────────────────────────────────────
       Pinta TODAS las cards del catálogo de ese scope, en orden: con dato, la card; sin dato, su
       marco con «todavía no». `host` opcional: si la pestaña tiene sitio reservado se pinta ahí;
       si no, se cuelga al final de la .insight-page de la pestaña (y si no existe, se crea). */
    async _renderVera4(body, scope, host) {
      if (!body) return;
      this._vera4Cache = {};                     // repintado = lectura fresca
      this._v4Paleta();
      const r = await this._loadVera4(scope);
      if (!body.isConnected) return;
      const datos = r.datos;
      this._vera4At = datos ? datos.createdAt : null;
      const porTipo = new Map();
      (datos ? datos.cards : []).forEach((c) => { if (c && VERA4[c.type] && !porTipo.has(c.type)) porTipo.set(c.type, c); });
      const texto = this._faltaTexto(r, (FALTA_TXT[scope] || FALTA_TXT.mi_marca)());
      const html = Object.keys(VERA4)
        .filter((tipo) => !VERA4[tipo].aparte && VERA4[tipo].tab && this._v4Cabe(tipo, scope))
        .map((tipo) => (porTipo.has(tipo) && this._vera4CardHtml(porTipo.get(tipo))) || this._v4Pendiente(tipo, texto))
        .join('');

      let destino = host || body.querySelector('.vera4');
      if (!destino) {
        // Sin sitio reservado: dentro de la página de la pestaña, para heredar su ancho.
      ...
      this._pintar(destino, html);
      // Color de marca en los paneles que lo piden (mismo ayudante que el resto).
      destino.querySelectorAll('[data-panel-marca]').forEach((el) => this._vestirPanelDeMarca?.(el));
      // Los instrumentos con canvas se pintan cuando el HTML ya está en el DOM.
      await this._paintVera4Charts(destino);
    },

    /** La card que Vera no escribió: mismo marco, mismo rótulo, mismo ancho; el cuerpo dice qué falta. */
    _v4Pendiente(tipo, texto) {
      const meta = VERA4[tipo];
      const esc = (s) => this._esc(s);
      return `
        <section class="v4-card v4-card--pendiente${ANCHAS.includes(meta.layout) ? ' v4-card--ancha' : ''}" data-v4="${esc(tipo)}">
          ...
          <div class="v4-body">${this._todaviaNo(meta.label(), texto)}</div>
        </section>`;
    },
    ...
    /* ── Propuestas de oportunidad: la card `aparte` de Tendencias. Solo se pinta en su hueco
       bajo el calendario (#tendPropuestas); v1 nunca lo tuvo en la ruta viva, así que hoy no sale. */
    async _renderPropuestasFecha(body) {
      if (!body) return;
      const hueco = body.querySelector('#tendPropuestas');
      if (!hueco) return;
      const r = await this._loadVera4('tendencias');
      const card = r.datos && r.datos.cards.find((c) => c && c.type === 'propuestas_fecha');
      if (!card || !this._v4Cabe('propuestas_fecha', 'tendencias')) { this._pintar(hueco, ''); return; }
      let cuerpo = '';
      try { cuerpo = this._v4Propuestas(card); } catch (e) { console.warn('[Vera4] propuestas:', e && e.message); }
      this._pintar(hueco, cuerpo ? `<div class="vera4">${this._v4Marco(card, VERA4.propuestas_fecha, cuerpo)}</div>` : '');
    },

    /* ══ LA INTUICIÓN DEL TAB ═══════════════════════════════════════════════
       Cada tablero tiene la SUYA: en Competencia el sujeto es el rival, en Tendencias el mercado,
       en Estrategia la jugada (la de Mi Marca es cards.v2 y la pinta BrandGrid). Arriba del todo
       en la columna izquierda: primero lo que Vera ve y nadie más puede decir. Competencia tiene
       hueco reservado (#cgridIntuicion); Tendencias y Estrategia la insertan como PRIMER hijo de
       su columna. Sin Intuición escrita, la banda sale igual, con su rótulo y «todavía no». */
    async _renderIntuicionDelTab(body, scope) {
      if (!body || !INTU_TAB[scope]) return;
      const previa = body.querySelector('.vera-intu-tab');
      const r = await this._loadVera4(scope);
      if (!body.isConnected) return;
      const card = r.datos && r.datos.cards.find((c) => c && c.type === 'intuicion');
      let html = card && this._v4Cabe('intuicion', scope) ? this._v4IntuicionHtml(card, scope, r.datos.createdAt) : '';
      if (!html) html = this._v4IntuicionPendiente(scope, this._faltaTexto(r, (FALTA_TXT[scope] || FALTA_TXT.mi_marca)()));

      const wrap = document.createElement('div');
      wrap.className = 'vera-cards vera-intu-tab';
      this._pintar(wrap, html);
      ...
      if (reservado) {
        ...
        reservado.replaceChildren(wrap);
      } else {
        let col = null;
      ...
      try { this._acentuarIntuicion?.(wrap); } catch (e) { console.warn('[Vera4] acento de la Intuición:', e && e.message); }
    },

    /** La banda de Intuición sin lectura: su rótulo y su subtítulo, y qué falta. */
    _v4IntuicionPendiente(scope, texto) {
      const meta = INTU_TAB[scope];
      const esc = (s) => this._esc(s);
      return `
        <section class="vera-card vera-card--intuicion vera-card--pendiente" data-tone="neutral" data-v4="intuicion">
          ...
          <div class="vera-card-body">${this._todaviaNo(meta.kicker(), texto)}</div>
        </section>`;
    },

    /* La card v4 → el molde visual de la Intuición: cita, porqué, acierto/culpable y qué hacer,
       con los bloques que ya sabe pintar el tablero (_veraBlockHtml de Mi Marca). */
    _v4IntuicionHtml(card, scope, createdAt) {
      const esc = (s) => this._esc(s);
      const meta = INTU_TAB[scope];
      if (!meta || !card || !card.titulo || typeof this._veraBlockHtml !== 'function') return '';
      const blocks = [];
      if (card.de_donde) blocks.push({ type: 'quote', text: String(card.de_donde), source: card.lo_obvio ? String(card.lo_obvio) : '' });
      ...
      // Tono NEUTRO: toma el acento de la marca, el único color que esta pieza admite.
      if (card.que_hago) blocks.push({ type: 'callout', tone: 'neutral', icon: 'compass', title: __('Qué hacer con esto'), markdown: String(card.que_hago) });
      if (!blocks.length) return '';
      const inner = blocks.map((b, i) => this._veraBlockHtml(b, `intu-${scope}`, i)).join('');
      // La confianza tiene su propio vocabulario ('media' ≠ «Señal media») y va al pie, en minúscula.
    ...
    _v4Marco(card, meta, cuerpo) {
      const esc = (s) => this._esc(s);
      const ancho = ANCHAS.includes(meta.layout);
      return `
    ...
    _v4Heatmap(card) {
      ...
      const filas = perfiles.map((p, i) => `
        ...
        ${temas.map((t, j) => {
          ...
          return `<div class="v4-hm-c${v <= 0 ? ' is-cero' : ''}" data-var="--a:${a.toFixed(2)}"
      ...
      return `<div class="v4-hm" data-var="--cols:${temas.length}">
    ...
    _v4Apilada(card) {
      ...
      const leyenda = tonos.map((t, i) =>
        `<span class="v4-leg"><i data-bg="${esc(SERIES[i % SERIES.length])}"></i>${esc(t)}</span>`).join('');
        ...
        return `<div class="v4-ap-fila">
          ...
          <div class="v4-ap-barra" role="img" aria-label="${esc(tonos.map((t, i) =>
            ...
            ${tonos.map((t, i) => {
              ...
              if (pct <= 0) return '';
              return `<span class="v4-ap-seg" data-w="${pct.toFixed(1)}" data-bg="${esc(SERIES[i % SERIES.length])}"
    ...
    _v4Divergente(card) {
      ...
      const leyenda = escala.map((e, i) =>
        `<span class="v4-leg"><i data-bg="${esc(color(i))}"></i>${esc(e)}</span>`).join('');
      const filas = perfiles.map((p) => {
        ...
        const seg = (i) => (pc[i] > 0
          ? `<span class="v4-dv-seg" data-w="${pc[i].toFixed(1)}" data-bg="${esc(color(i))}"
        ...
        return `<div class="v4-dv-fila">
          ...
          <div class="v4-dv-pista" role="img" aria-label="${esc(escala.map((e, i) =>
            ...
            <div class="v4-dv-barra" data-var="--dx:${(50 - izq).toFixed(2)}%">
    ...
    _v4Indexadas(card) {
      ...
      const leyenda = series.map((s, i) =>
        `<span class="v4-leg"><i data-bg="${esc(SERIES[i % SERIES.length])}"></i>${esc(s.nombre || '')}</span>`).join('');
      return `<div class="v4-leyendas">${leyenda}</div>
    ...
    async _paintVera4Charts(raiz) {
      ...
      try { await this._ensureChartJs(); } catch (e) { console.warn('[Vera4] Chart.js:', e && e.message); return; }
      if (typeof window.Chart === 'undefined') return;
      const tinta = this._tintaCharts();
      canvas.forEach((cv) => {
        if (!cv.isConnected) return;
        let d;
        try { d = JSON.parse(cv.dataset.v4chart); } catch (e) { console.warn('[Vera4] serie ilegible:', e && e.message); return; }
        if (d.kind === 'dispersion') { this._v4PintaDispersion(cv, d, tinta); return; }
        this._reg(new window.Chart(cv.getContext('2d'), {
          type: 'line',
          data: {
            labels: d.meses,
            datasets: (d.series || []).map((x) => ({
              label: x.nombre, data: x.valores, borderColor: x.color, backgroundColor: x.color,
          ...
          options: {
            ...
            plugins: { legend: { display: false }, tooltip: tinta.tooltip },   // la leyenda ya está en HTML
            scales: {
              x: { grid: { display: false }, ticks: { color: tinta.tick, maxRotation: 0, autoSkip: true } },
              y: { grid: { color: tinta.grid }, ticks: { color: tinta.tick } },
            },
          },
        }));
      });
    ...
    _v4Enfasis(card) {
      ...
      return `<div class="v4-enf">${orden.map((m) => {
        ...
        return `<div class="v4-enf-fila${vacio ? ' is-vacia' : ''}">
          ...
          <div class="v4-enf-pista"><span class="v4-enf-barra" data-w="${(v / max * 100).toFixed(1)}"
    ...
    _v4Lineas(card) {
      ...
      return `<div class="v4-leyendas">${series.map((x) =>
        `<span class="v4-leg${x.codigo === dest ? ' is-dest' : ''}"><i data-bg="${
          esc(x.codigo === dest ? SERIES[0] : GRIS)}"></i>${esc(x.codigo || '')}</span>`).join('')}</div>
      <div class="v4-chart-wrap"><canvas id="${cid}" data-v4chart='${
        this._esc(JSON.stringify({ kind: 'lineas', meses: fechas,
          series: series.map((x) => ({ nombre: x.codigo, valores: x.valores,
            color: x.codigo === dest ? SERIES[0] : GRIS })) }))}'></canvas></div>
      ...
      const cols = meses.map((m, i) => {
        ...
        return `<div class="v4-bal-col" title="${esc(`${m}: ${Math.round(pc)}% construir`)}">
          <div class="v4-bal-barra">
            <span class="v4-bal-con" data-h="${pc.toFixed(1)}"></span>
            <span class="v4-bal-cos" data-h="${(100 - pc).toFixed(1)}"></span>
          </div><span class="v4-bal-mes">${esc(m)}</span></div>`;
      }).join('');
      return `<div class="v4-leyendas">
          <span class="v4-leg"><i class="v4-leg-con"></i>${esc(__('Construir marca'))}</span>
          <span class="v4-leg"><i class="v4-leg-cos"></i>${esc(__('Cosechar demanda'))}</span>
          <span class="v4-leg v4-leg--vara"><i></i>${esc(__('vara {v}% (Binet & Field)', { v: vara }))}</span>
        </div>
        <div class="v4-bal"><div class="v4-bal-vara" data-var="--vara:${Math.max(0, Math.min(100, vara))}%"></div>${cols}</div>
        ${this._v4Metodo(card)}
    ...
    _v4PintaDispersion(cv, d, tinta) {
      const t = tinta || this._tintaCharts();
      const corteColor = this._tok('--white-20');
      const ejeX = { type: 'linear', grid: { color: t.grid },
        ticks: { color: t.tick },
        title: { display: true, text: d.ejes.x, color: t.tick, font: { size: 10 } } };
      ...
      this._reg(new window.Chart(cv.getContext('2d'), {
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: { ...t.tooltip, callbacks: { label: (c) => {
              const p = c.raw || {};
      ...
      const barra = (etq, v, color) => {
        ...
        return `<div class="v4-desc-fila">
          ...
          <div class="v4-desc-pista">
            <span class="v4-desc-barra${val < 0 ? ' is-neg' : ''}" data-w="${w.toFixed(1)}" data-bg="${esc(color)}"></span>
          </div>
    ...
    _v4TendenciaOModa(card) {
      ...
      const spark = (vals) => {
        ...
        return `<svg class="v4-spark" viewBox="0 0 100 28" preserveAspectRatio="none" aria-hidden="true">
          <polyline points="${pts}" fill="none" stroke="${esc(SERIES[0])}" stroke-width="2" vector-effect="non-scaling-stroke"/></svg>`;
      };
