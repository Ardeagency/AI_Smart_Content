 ...
 * con «Lo hice» → acted_on), las tendencias vivas (intel.tendencias_vivas / trends),
...
<div class="insight-page tablero" id="tableroPage">
  ...
  _pintarLecturas() {
    ...
    host.innerHTML = [
      ...
      atendidas.length ? `<details class="tablero-atendidas"><summary>${this.escapeHtml(__('Atendidas ({n})', { n: atendidas.length }))}</summary>${atendidas.slice(0, 6).map((l) => L.lectura(l)).join('')}</details>` : '',
    ].join('');
  }
  ...
  _bind() {
    ...
    host.addEventListener('click', async (e) => {
      ...
      try {
        ...
        if (typeof window.showToast === 'function') window.showToast(__('Marcada como atendida.'));
      } catch (err) {
        btn.disabled = false;
