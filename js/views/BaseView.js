...
class BaseView {
  ...
  /**
   * Gancho que el render llama tras pintar la vista. El shell (js/shell/Shell.js) es dueño de la
   * cabecera: marca, avatar y cuenta salen de mi_contexto(). Las vistas lo sobreescriben para afinar
   * el contexto (updateHeaderContext) o para no tener shell (páginas de acceso).
   * Antes pintaba la cabecera de v1 (.main-header, #headerUser…), que ya no existe, y pedía
   * profiles?select=id,full_name,email,role: 400 en cada vista de producción (25/09), porque la
   * base nueva no tiene email ni role en profiles.
   */
  async updateHeader() {}
