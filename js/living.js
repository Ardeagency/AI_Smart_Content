...
class LivingManager {
    ...
    async renderHistorySection() {
        ...
        this.applyJustifiedLayout(container);
    ...
    /**
     * Justified-rows layout (Flickr-style). Para cada item lee el aspect ratio
     * del media (img.naturalWidth/Height o video.videoWidth/Height) y agrupa los
     * items en filas que llenan exactamente el ancho del container. Cada fila
     * tiene altura uniforme; cada item un ancho = ratio * altura_fila.
     *
     * La última fila NO se estira al ancho completo: se queda a TARGET_HEIGHT
     * para que pocos items finales no se vean exageradamente grandes.
     *
     * Re-layout en:
     *  - cada renderHistorySection() (al inicio)
     *  - cada load de imagen (porque inicialmente usamos ratio default = 1)
     *  - resize de ventana (debounce 150ms)
     */
    applyJustifiedLayout(container) {
        const TARGET_HEIGHT = 280;
        const GAP = 8;
        const MAX_RATIO = 3;   // recorta horizontales ultra-anchas
        const MIN_RATIO = 0.4; // recorta verticales ultra-altas

        const layout = () => {
            const grid = container.querySelector('.living-masonry-grid') || container;
            const items = Array.from(grid.querySelectorAll('.living-masonry-item'));
            ...
            const entries = items.map((item) => {
                ...
                // Clamp para que ningún item domine la fila o sea diminuto.
            ...
            const flushRow = (isLast) => {
                ...
                const naturalRowW = rowRatioSum * TARGET_HEIGHT;
                // height ideal para que sum(ratio * height) = targetRowW
                let height;
                if (isLast) {
                    // Estira la última fila solo si está >50% llena al target height.
                    // Con menos items dejamos altura target (aligned-left, sin estirar).
                    const fillRatio = (naturalRowW + totalGap) / containerW;
                    height = fillRatio > 0.5 ? targetRowW / rowRatioSum : TARGET_HEIGHT;
                } else {
            ...
            for (const e of entries) {
                const projectedItemW = e.ratio * TARGET_HEIGHT;
                const projectedRowW = rowRatioSum * TARGET_HEIGHT + row.length * GAP + projectedItemW;
        ...
        // Re-layout cuando cada imagen termine de cargar (entrega su naturalWidth/Height).
        ...
        if (!this._justifiedResizeBound) {
            this._justifiedResizeBound = true;
            let t = null;
            window.addEventListener('resize', () => {
                if (t) clearTimeout(t);
                t = setTimeout(() => {
                    const c = document.getElementById('livingHistoryContent');
                    if (c) this.applyJustifiedLayout(c);
                }, 150);
            }, { passive: true });
        }
    }
