...
class StudioManager {
    ...
    showTestScenesButton() {
        const canvasArea = document.querySelector('.canvas-area');
        if (canvasArea) {
            canvasArea.innerHTML = `
                <div class="guiones-container">
                    <div class="guiones-header">
                        <h2>🧪 Prueba de Escenas</h2>
                        <p>Haz clic en el botón para probar el webhook de escenas con un guión de prueba</p>
                    </div>
                    <div class="test-scenes-container">
                        <button class="btn btn-primary btn-large" onclick="window.studioManager.testScenesWebhook()">
                            🧪 Probar Webhook de Escenas
                        </button>
                        <p class="test-description">
                            Este botón enviará un guión de prueba al webhook de escenas para verificar que funciona correctamente.
                        </p>
                    </div>
                </div>
            `;
    ...
    async testScenesWebhook() {
        try {
            console.log('=== INICIANDO PRUEBA DE WEBHOOK DE ESCENAS ===');
            
            // Mostrar loading
            this.showLoading('Probando webhook de escenas...');
            
            // Crear guión de prueba
            const mockGuion = {
                tipo_guion: "Enfoque Persona - Test",
                titulo_sugerido: "Guión de Prueba para Escenas",
                clips: [
                    {
                        clip_numero: "Clip 1",
                        escena: "Una persona joven y dinámica se despierta con energía, estirándose en su cama mientras la luz del sol entra por la ventana. Su rostro muestra determinación y motivación para comenzar el día.",
                        voz: "¡Buenos días! Hoy es un día perfecto para alcanzar tus metas y superar cualquier desafío que se presente."
                    },
                    {
                        clip_numero: "Clip 2", 
                        escena: "La persona se levanta de la cama y camina hacia el espejo del baño, mirándose con confianza mientras se prepara para el día. Su reflejo muestra seguridad y positivismo.",
                        voz: "Cada mañana es una nueva oportunidad para ser la mejor versión de ti mismo. Recuerda que tienes todo lo necesario para triunfar."
                    },
                    {
                        clip_numero: "Clip 3",
                        escena: "La persona sale de su habitación y camina por un pasillo iluminado, con una sonrisa genuina en su rostro. Su postura es erguida y su paso es decidido.",
                        voz: "No importa qué obstáculos encuentres en el camino, siempre recuerda que eres más fuerte de lo que crees y más capaz de lo que imaginas."
                    }
                ]
            };
            
            console.log('Guión de prueba creado:', mockGuion);
            
            // Generar configuración completa (misma que para guiones reales)
            const configData = await this.generateConfigJSON();
            console.log('Configuración generada para prueba:', configData);
            
            // Crear datos finales con el guión de prueba
            const finalData = {
                ...configData,
                selected_guion: {
                    index: 0,
                    tipo_guion: mockGuion.tipo_guion,
                    titulo: mockGuion.titulo_sugerido,
                    clips: mockGuion.clips
                }
            };
            
            console.log('Datos finales para prueba de webhook:', finalData);
            ...
            console.error('Error en prueba de webhook de escenas:', error);
            this.showError(`Error en prueba: ${error.message}`);
        } finally {
            this.hideLoading();
        }
    }
